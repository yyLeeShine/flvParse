...
import (
	"bufio"
	"encoding/json"
	"github.com/yyleeshine/flvParse/aacParse"
	"github.com/yyleeshine/flvParse/av"
	"github.com/yyleeshine/flvParse/bits/pio"
	"github.com/yyleeshine/flvParse/flv/flvio"
	"github.com/yyleeshine/flvParse/h264Parse"
	"fmt"
	"io"
	//"time"
	//"encoding/hex"
	//"encoding/hex"
	//"encoding/hex"
	"github.com/yyleeshine/flvParse/amf"
	//"encoding/hex"
	"encoding/hex"
)
...
type Prober struct {
	...
	CachedPkts                     []av.Packet // Packet stores compressed audio/video/mesg data.
}

func (self *Prober) Empty() bool {// 查看packet的长度
	return len(self.CachedPkts) == 0
}

func (self *Prober) PopPacket() av.Packet {// 拿出packet
...
type flvHeadMsg struct {// flv头信息
	FlvFlag string // Flag byte是流的信息，倒数第一bit是1表示有视频（0x01），倒数第三bit是1表示有音频（0x4），
	TagHeaderLeng int
	HeadData string
...
type TagMsg struct {
	TagType int // 是音频还是视频或者是脚本
	TagHeaderLeng int //tag header的长度
	TagDataLeng int //tag data的长度
	TagNum int64 //该tag的数量
	CodecType string
...
func(self *Demuxer) FlvParse()(err error){
	if _, err = io.ReadFull(self.bufr, self.b[:flvio.FileHeaderLength]); err != nil {//先读取FLV Header的内容，其长度为9
		return
	}
	...
	for {
		...
		if tag, timestamp, err = flvio.ReadTag(self.bufr, self.b); err != nil {//先得到tag 时间戳
			return
		}
