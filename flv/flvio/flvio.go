...
func (self *Tag) videoParseHeader(b []byte) (n int, err error) {
	...
	flags := b[n]
	self.FrameType = flags >> 4//Frame的类型
	self.CodecID = flags & 0xf //编码方式
	n++
...
func ParseTagHeader(b []byte) (tag Tag, ts int32, datalen int, err error) {
	 ...
	 */
	tagtype := b[0]//第一个字节是type
	//tag data len 4 bytes
	datalen = int(pio.U24BE(b[1:4]))//tagdate的长度
	switch tagtype {
	...
	var tslo uint32
	var tshi uint8
	tslo = pio.U24BE(b[4:7])//timestamp
	tshi = b[7]//timestamp_ex
	ts = int32(tslo | uint32(tshi)<<24)//得到时间戳

	return
}

func ReadTag(r io.Reader, b []byte) (tag Tag, ts int32, err error) {
	if _, err = io.ReadFull(r, b[:TagHeaderLength]); err != nil {// 读取tagHeader的部分，一共十一个字节
		return
	}
	//tag header
	//fmt.Println("flv tag header:",hex.Dump(b[:TagHeaderLength]))
	var datalen int
	if tag, ts, datalen, err = ParseTagHeader(b); err != nil {// tagType、时间戳、数据区域长度
		return
	}
	...
	data := make([]byte, datalen)//读取相应长度的数据
	if _, err = io.ReadFull(r, data); err != nil {
		return
	}

	var n int
	if n, err = (&tag).ParseHeader(data); err != nil {//解析data的header
		return
	}
	tag.Data = data //视频数据
	tag.DataPos = n //视频数据的位置
